...
\chapter{Dynamic parallelism detection}
	...
	\section{Loop level parallelism}
		...
		Parallelism is a general topic that can apply to all sorts of code in all sorts of ways. We will only be discussing a very specific kind of parallelism: loop level parrelelism. This type of parrelelism is common, easy to detect, scales well on massively parrelel hardware, and simple for programmers to reason about. 
		
		But before we get into the details of why we care so much about loop level parrelelism, lets go into some detail about what it is.
		
		\subsection{What is Loop level parrelelism?}
		
		In a vauge sense, Loop level parrelelism occurs when each iteration of the loop can be run in parrellel with all the other iterations. 
...
for(int i = 1; i < n; i++){
	B[i] = B[i-1] + A[i];
		...
		\subsection{Desirable qualities of loop level parelleism}
		
		Loop level parrelelism is common, easy to detect, scales well on massively parrelel hardware, and simple for programmers to reason about. 
		
		The most important thing about loop level parrelelism is that assuming the loop is reasonably large, it can be scaled to massively parrelell hardware like GPUs or other kinds of accelerators. 
		
		Lastly, loop level parrellelism is easy to detect. The next section will give a simple algorithm to detect this. Other kinds of massive parrelleism require more sophisticated analysis. 
		
		\subsection{Dynamic Loops}
		
		Loops have a very clean and simple structure in high languages like C. However, we are interested in dynamic analysis of loops, which means we will have to detect and analyze loops in machine code. 
		
		
	\section{Loop level parallelism and the base algorithm}
	
		We will be entirely focused on  detecting a single type of parallelism: loop level parallelism. This type of parallelism is easy to detect, and common in real code. 
		
		\subsection{Formalizing parallelism}
	...
	\section{Need for a new dynamic parallelism detection tool}
		...
		%Now, there a massively parallel algorithm that does compute the same answer. However, we will not concern ourselves with this because it is impossible to detect this in general. The algorithm choice will still need to be made by hand. Rather, we want to be able to detect whether the particular algorithm is 
