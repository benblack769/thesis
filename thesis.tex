...
for(int i = 1; i < n; i++){
	B[i] = B[i-1] + A[i];
		...
		A huge amount of work in research and tooling has been devoted to loop level parallelism. 
		
		LLP for four reasons: it is common, scales well on massively parallel hardware, simple for programmers to reason about, and easy to detect. 
...
...
	...
	\section{The baseline algorithm: The pairwise method}
		...
		Now, there is an importance difference between memory and dependencies. Memory can sometimes be reused without creating dependencies. In particular, if memory is written to before it is read in a loop iteration, then it is "killed", and further reads in that iteration will not be tracked for dependencies. 
