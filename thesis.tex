...
for(int i = 1; i < n; i++){
	B[i] = B[i-1] + A[i];
		...
		Of course, loops are very common in all low level code. A large number of these loops are massively parallel CITATION!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	\section{Loop level parallelism and the base algorithm}
		...
		\subsection{The pairwise method}
			...
			The baseline algorithm we use for detecting loop level parallelism is called the pairwise method. It 
