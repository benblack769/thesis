...
\chapter{Dynamic parallelism detection}
	\section{Data Dependency graphs}
		...
		One important example of a massively parrelell algorithm is summing. When you have an array $A_1,A_2,...,A_n$ of numbers and you sum them, you can get a log depth dependency graph with a divide and conquer algorithm like the following:
		...
		However, the most obvious algorithm does not work like this at all. The standard algorithm, each dependence.
		
		For example, when summing an array, 
	...
	\section{Pairwise method}
		Note that any naive implemention of actually constructing a data dependence tree from a real computation will take $\Omega(n)$ space, where $n$ is the number of steps of the computation. Even for quick computations, this memory requirement quickly grows astronomical, and analysis becomes unusably slow. So instead, we will look at special cases and approximations to get what we need. 
